...
const AddDelivery = () => {
  ...
  const AddDeliveryFetch = (e) => {
    fetch(
      `https://eshopsmart.herokuapp.com/api/delivery/store?name=${titleInput}&time=${keywordsInput}&price=${priceInput}&terms=${descriptionInput}`,
      {
        method: "POST",
        headers: {
    ...
    <CRow>
      <CCol xs="12" md="12">
        <CCard>
          <CCardHeader>
            {deliveryAdded ? (
              <CAlert color="primary">Pristatymo metodas pridėta!</CAlert>
            ) : null}
            {requestError ? (
              ...
              <CFormGroup row>
                ...
                <CCol xs="12" md="9">
                  <CInput
                    ...
                    id="email-input"
                    name="email-input"
                    placeholder="Pristatymo laikas pvz.: 1-2 d.d."
                    autoComplete="tag"
                    value={keywordsInput}
